...
for sub in sublist:
    try:
        subs.append(sub)
        embeddings.append(np.load(data_folder+f'/embedding_dense_emb.{sub}.ses-001.control.npy'))
        embeddings.append(np.load(data_folder+f'/embedding_dense_emb.{sub}.ses-001.meditation.npy'))
        embeddings.append(np.load(data_folder+f'/embedding_dense_emb.{sub}.ses-001.hypnose.npy'))
        print(sub)
    except:
        print(sub)
...
savemat(data_folder+'/cont_med_hyp_group_embedding_new.mat', mdict={'emb': realigned, 'subs': subs})

    
f = loadmat(data_folder+'/cont_med_hyp_group_embedding_new.mat')
print('Group matrix succeded and saved in {}'.format(data_folder+'/cont_med_hyp_group_embedding_new.mat'))
