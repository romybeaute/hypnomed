...
class embedding:
    ...
    def get_expertise(self, group):
        ...
        if group == 'MH': #group with meditation first
            sub_ids = list(self.sub['Subs_ID'][self.expertise.MH == 0])
            self.remove_sub(sub_ids)
        elif group == 'HM':  #group with hypnosis first
            sub_ids = list(self.sub['Subs_ID'][self.expertise.HM == 0])
            self.remove_sub(sub_ids)
        elif group != 'all':
            sys.exit(
                'error in p.group, group not recognized ! Should be MH, HM or all')


    def get_states(self, states_wanted):
        #keep states wanted
        #states should be one of 'control', 'meditation' or 'hypnose'
